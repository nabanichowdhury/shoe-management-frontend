...
const SalesReport = () => {
  ...
  useEffect(() => {
    ...
    filterSales(); // This function updates the state based on `selectedOption`
  }, [selectedOption, isLoading]);

  if (isLoading) return <p>Loading...</p>;

  const totalEarned = filteredSales.reduce(
        ...
        <div>
          ...
          <div className="overflow-x-auto">
            <table className="table table-xs">
              ...
              <tbody>
                {filteredSales?.map((d: any, index: number) => (
                  <tr key={d._id}>
                    <th>{index + 1}</th>
                    <td>{d.buyer}</td>
                    <td>{d.soldItem}</td>
